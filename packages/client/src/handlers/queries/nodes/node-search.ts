...
import { NodeSearchQueryInput } from '@colanode/client/queries/nodes/node-search';
...
{
  public async handleQuery(input: NodeSearchQueryInput): Promise<LocalNode[]> {
    const rows =
  ...
  private isRelevantType(input: NodeSearchQueryInput, type: NodeType): boolean {
    return (
  ...
  private async fetchNodes(input: NodeSearchQueryInput): Promise<SelectNode[]> {
    const workspace = this.getWorkspace(input.userId);
